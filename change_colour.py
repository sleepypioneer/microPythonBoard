...
def print_to_display():
    ...
    lcd.set_pos(20, 20)
    lcd.set_text_color(lcd.rgb(*colours[calculate_color(temp)]), lcd.rgb(0, 0, 0))
    lcd.set_font(1, 0, 0, 0, scroll=1)
    lcd.write("Temperature(°C)")
    lcd.set_pos(50, 50)
    lcd.set_font(1, 0, 0, 0, 0)
    lcd.write("%.2f" % (temp))
    lcd.set_text_color(lcd.rgb(255,255,255), lcd.rgb(0, 0, 0)) # star unpacks the colour tuple into the rgb
    lcd.set_pos(30, 100)
    lcd.set_font(1, 0, 0, 0, scroll=1)
    lcd.write("Humidity(%)")
    lcd.set_pos(50, 130)
